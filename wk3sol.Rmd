...
### 1.  what is the (absolute) difference between the average GDP per capita of Americas and Asia in year 2002?
```{r}
...
* another more dplyr focused way to do it: 
...
### 2. which continent had the highest GDP per capita in year 2002?
...
* Create a rank - group_by does tricky things! Need to ungroup to get arrange to work right.
* Can use top_n() to return top row
...
### 3. Which continent had the lowest GDP per capita in year 2002?
...
### 4 which country had the highest GDP per capita in year 1967?
...
### 5. which country had the highest life expectancy in year 2002 and what was the value of it?
...
* 9 add a log(GDP per capita in 1967) column to yr1967CountGdpPercap
...
* Graph (scatter plot) x: Log(GDP per capita per continent in 1967), y: Life Expectancy per continent in 1967
...
* Graph (line plot) x: year, y: Log(GDP per capita per continent)
...
```{r}
#Exercise 1. Load the revenue.csv le into your R project.
...
```{r}
#Exercise 2. By using gather(), `gather' the quarter revenues into Quarter and Revenue.
longRev <- revenue %>% gather(Quarter, Revenue, Qtr.1:Qtr.4)
head(longRev)
#Exercise 3. By using separate(), `separate' each of the Quarter strings into Time Interval and Interval ID
separateRev <- longRev %>% separate(Quarter, c("Time_Interval", "Interval_ID"))
head(separateRev)
#Exercise 4. By using unite(), re-unite the Time Interval and Interval ID and re-create the original Quarter variable.
uniteRev <- separateRev %>% unite(Quarter, Time_Interval, Interval_ID, sep=".")
head(uniteRev)
#Exercise 5. By using spread(), restore the original structure of the dataframe.
